...
WHAT IS LEFT OUT, and why (D-032): small civil airports (42,698), heliports (23,135),
seaplane bases and balloonports are excluded. Including them is ~66,000 extra markers
worldwide and 321 inside 2 degrees of Mobile alone - the unreadable mat this project exists
to avoid. Large and medium airports are where the traffic ADS-B actually shows us operates.
...
def build_airports(csv_path: Path) -> list[list]:
    """[lat, lon, code, kind] for large + medium airports and anything named as military."""
    out: list[list] = []
    ...
    with csv_path.open(encoding="utf-8", newline="") as f:
        for row in csv.DictReader(f):
            ...
            military = bool(MILITARY_NAME.search(row["name"]))
            if military:
                kind = KIND_MILITARY
            elif row["type"] in kind_by_type:
                kind = kind_by_type[row["type"]]
            else:
                continue
...
def build_cities(geojson_path: Path) -> list[list]:
    ...
    for feat in data["features"]:
        ...
        out.append([round(lat, 4), round(lon, 4), name, int(p.get("scalerank", 10))])
